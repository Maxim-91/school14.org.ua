...
class JA_IconMenu extends JA_Base {
	var $parent = null;
	...
	function beginMenu(){
		...
		?>		
		<script language="javascript" type="text/javascript" src="<?php echo $this->getParam('menupath');?>/ja_iconmenu/ja.iconmenu.js"></script>
	...
	function genMenuItem(&$row, $level, $pos){
		//global $Itemid, JRUI::base(), $mainframe;
		$txt = '';
		...
		switch ($row->type) {
			...
			case 'content_typed':
				// load menu params
				$menuparams = new $this->createParameterOjbect( $row->params, $mainframe->getPath( 'menu_xml', $row->type ), 'menu' );
		...
		if ( $this->getParam('level')==0) {
			...
			//echo $menu_image.'aaaaa'; 
			if ( ( $menu_image <> '-1' ) && $menu_image ) {
				//$mouseaction = ' onmouseover="iconmenu_over(\''.($pos+1).'\')" onmouseout="iconmenu_out(\''.($pos+1).'\')"';
				$clsactive = $active ? " class=\"active\"":"";
				//$txt = '<img src="'. JRUI::base() .'/images/stories/'. $menu_image .'" border="0" alt="'. $row->name .'" onmouseover="iconmenu_over(\''.($pos+1).'\')" onmouseout="iconmenu_out(\''.($pos+1).'\')" width=\''.$this->getParam('icon_small').'\' height=\''.$this->getParam('icon_small').'\''.$clsactive.' />';
				$msie='/msie\s(5\.[5-9]|[6]\.[0-9]*).*(win)/i';
				
				if( !isset($_SERVER['HTTP_USER_AGENT']) ||
					!preg_match($msie,$_SERVER['HTTP_USER_AGENT']) ||
					preg_match('/opera/i',$_SERVER['HTTP_USER_AGENT'])) {
					$txt = '<img src="'. JURI::base() .'/images/stories/'. $menu_image .'" border="0" hspace="0" vspace="0" alt="'. $row->name .'"'.$clsactive.' style="width: '.$this->getParam('icon_small').'px; height: '.$this->getParam('icon_small').'px;" />';
				...
				$txt .= '<img src="'. JURI::base() .'images/blank.png" border="0" alt="'. $row->name .'" width=\'1\' height=\'1\' />';
			}
		}
